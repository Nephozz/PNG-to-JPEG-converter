...
*/
pub struct Image<P> where P: PixelTrait, {
    width: u32,
    height: u32,
    data: DMatrix<P>,
}
...
impl<P> Image<P> where P: PixelTrait {
    ...
    pub fn from_png(image: ImageBuffer<Rgba<u8>, Vec<u8>>) -> Self {
...
*/
pub fn split(image: ImageBuffer<Rgba<u8>, Vec<u8>>) -> (Image<Luma<u8>>, Image<Cb<u8>>, Image<Cr<u8>>) {
    // to be implemented
    println!("Not implemented yet");

    let y_image = Image::<_>::from_png(image);
    let cb_image = Image::<_>::from_png(image);
